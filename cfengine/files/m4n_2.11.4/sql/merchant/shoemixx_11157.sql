...
/* July 1 2010, Jan, ticket: #5467
Zou jij voor adverteerder Shoemixx (ID 11157) de volgende staffel kunnen aanmaken (ingangsdatum vandaag).

Leadprogramma 2028: 0-9 sales --> 7% commissie
Leadprogramma 4576: 10-39 sales --> 8 commissie
Leadprograma: 4577: 40+ sales --> 9 commissie

(Het huidige leadprogramma dat nu draait is alleen 2028 met de standaard 7% commissie. Deze wordt dus nu omgezet naar een staffel.)
...
UPDATE
	lead
SET
	lead_program_id = 4576
WHERE
	lead_program_id IN (2028) AND
	status IN ('ACCEPTED', 'TO_BE_APPROVED', 'ON_HOLD') AND
	payment_period_id IN (SELECT id FROM payment_period WHERE processed = false) AND
...
UPDATE
	lead
SET
	lead_program_id = 4577
WHERE
	lead_program_id IN (2028, 4576) AND
	status IN ('ACCEPTED', 'TO_BE_APPROVED', 'ON_HOLD') AND
	payment_period_id IN (SELECT id FROM payment_period WHERE processed = false) AND
	created >= '2010-07-01' AND
	click_created < '2011-01-01' AND
