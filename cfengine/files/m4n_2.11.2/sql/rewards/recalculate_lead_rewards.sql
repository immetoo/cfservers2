	...
	(
		...
		FROM
			...
			(
				...
				WHERE 
					lead_reward.id = lead_reward_duration.lead_reward_id
			) AS lead_reward ON (
				...
				LEFT JOIN 
			lead_reward_multi_tier ON (lead_reward.id = lead_reward_multi_tier.id) 
				LEFT JOIN 
			lead_reward_tier ON (lead_reward_multi_tier.id = lead_reward_tier.lead_reward_multi_tier_id)
				LEFT JOIN 
			lead_reward_percentage ON (lead_reward.id = lead_reward_percentage.id OR lead_reward_tier.lead_reward_id = lead_reward_percentage.id)
				LEFT JOIN 
			lead_reward_fixed ON (lead_reward.id = lead_reward_fixed.id OR lead_reward_tier.lead_reward_id = lead_reward_fixed.id)
		WHERE
			(
				(
