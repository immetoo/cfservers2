...
CREATE TABLE statistics_sliding_per_affiliate_temp (
	id									BIGSERIAL NOT NULL,
	affiliate_id 						BIGINT NOT NULL, -- should be foreign key, but doesn't work now since souce table contains invalid id (0)
...
INSERT INTO statistics_sliding_per_affiliate_temp (
 	affiliate_id,
	status,
	cost_total,
	reward_total,
...
FROM	
    ...
    /* SUBQUERY last_payment_period */ (
        ...
        GROUP BY 
             affiliate_id, 
             status
    ) AS last_payment_period USING (affiliate_id, status)
    ...
    /* SUBQUERY current_payment_period */ (
        ...
        GROUP BY 
            affiliate_id, 
            status
    ) AS current_payment_period USING (affiliate_id, status)
	...
	/* SUBQUERY last_week */ (
		...
		GROUP BY 
			affiliate_id, 
			status
	) AS last_week USING (affiliate_id, status)
	...
	/* SUBQUERY yesterday */ (
		...
		GROUP BY 
			affiliate_id, 
			status
	) AS yesterday USING (affiliate_id, status);

BEGIN;
	DROP TABLE statistics_sliding_per_affiliate;
	ALTER TABLE statistics_sliding_per_affiliate_temp RENAME TO statistics_sliding_per_affiliate;		    
COMMIT;

CREATE INDEX statistics_sliding_per_affiliate_affiliate_id_idx ON statistics_sliding_per_affiliate USING BTREE (affiliate_id);
CREATE INDEX statistics_sliding_per_affiliate_status_idx ON statistics_sliding_per_affiliate USING BTREE (status);