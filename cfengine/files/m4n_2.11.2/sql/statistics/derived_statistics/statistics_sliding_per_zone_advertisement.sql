CREATE TABLE statistics_sliding_per_zone_advertisement_temp (
...
INSERT INTO statistics_sliding_per_zone_advertisement_temp (
	zone_id,
	advertisement_id,
	status,
	cost_total,
...
FROM	
    ...
    /* SUBQUERY last_payment_period */ (
        ...
        GROUP BY 
            ...
            advertisement_id, 
            status
    ) AS last_payment_period USING (zone_id, advertisement_id, status)
    ...
    /* SUBQUERY current_payment_period */ (
        ...
        GROUP BY 
            ...
            advertisement_id, 
            status
    ) AS current_payment_period USING (zone_id, advertisement_id, status)
	...
	/* SUBQUERY last_week */ (
		...
		GROUP BY 
			...
			advertisement_id, 
			status
	) AS last_week USING (zone_id, advertisement_id, status)
	...
	/* SUBQUERY */ (
		...
		GROUP BY 
			...
			advertisement_id, 
			status
	) AS yesterday USING (zone_id, advertisement_id, status);

BEGIN;
	DROP TABLE statistics_sliding_per_zone_advertisement;
	ALTER TABLE statistics_sliding_per_zone_advertisement_temp RENAME TO statistics_sliding_per_zone_advertisement;		    
COMMIT;

CREATE INDEX statistics_sliding_per_zone_advertisement_zone_id_idx ON statistics_sliding_per_zone_advertisement USING BTREE (zone_id);
CREATE INDEX statistics_sliding_per_zone_advertisement_advertisement_id_idx ON statistics_sliding_per_zone_advertisement USING BTREE (advertisement_id);
CREATE INDEX statistics_sliding_per_zone_advertisement_status_idx ON statistics_sliding_per_zone_advertisement USING BTREE (status);

-- ALTER TABLE statistics_sliding_per_zone_advertisement ADD CONSTRAINT advertisement_zone_status_unique_key UNIQUE(advertisement_id, zone_id, status);